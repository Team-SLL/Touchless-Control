...
import com.teamSLL.mlkit.camera.BitmapUtils;
import com.teamSLL.mlkit.camera.CameraImageGraphic;
import com.teamSLL.mlkit.camera.FrameMetadata;
import com.teamSLL.mlkit.camera.GraphicOverlay;
import com.teamSLL.mlkit.camera.InferenceInfoGraphic;
import com.teamSLL.mlkit.camera.ScopedExecutor;
...
/**
 * Abstract base class for vision frame processors. Subclasses need to implement  to define what they want to with the detection results and
...
public abstract class VisionProcessorBase<T> implements VisionImageProcessor {
    ...
    // -----------------카메라를 통해 실시간으로 분석 및 해당 화면을 제공 하는 함수
    @Override
    ...
    public void processImageProxy(ImageProxy image, GraphicOverlay graphicOverlay) {
        ...
        // 비트맵은 단지 출력하기 위함. 얼굴 분석의 기능과는 관련없음
        Bitmap bitmap = null;
    ...
    private Task<T> requestDetectInImage(
            ...
            long frameStartMs) {
        // 리스너를 만들 때 detectInImage 이게 분석하는 함수
        // detectInImage는 FaceDetectorProcessor에서 재정의되어 mlkit의 함수를 이용해 진행한다. 그 결과만 받아서 출력함
        return setUpListener(
                detectInImage(image), graphicOverlay, originalCameraImage, shouldShowFps, frameStartMs);
    }

    // 분석이 끝난 인풋이미지를 가지고 GraphicOverlay에 저장
        ...
        return task.addOnSuccessListener(
                ...
                results -> {
                    ...
                    if (originalCameraImage != null) {
                        // graphicOverlay.add 가 다음의 영상을 겹쳐서 출력하는 용
                        // 얼굴 이미지를 출력하는데 사용됨
                        graphicOverlay.add(new CameraImageGraphic(graphicOverlay, originalCameraImage));
                    }
                    // FaceDetectionProcessor의 Success에서 재정의되어 출력
    ...
    // FaceDetectorProcessor에서 재정의되어 분석
